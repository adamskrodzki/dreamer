...
Deno.test("CORRECTED: Basic dependency resolution - project with N dependencies resolves to N+1 tasks", () => {
  ...
  assertEquals(
    plan.tasks.length,
    3,
    "Should execute exactly N+1 tasks (dependencies + current project)",
  );
  ...
  assertEquals(
    taskIds.includes("./project-d:test"),
    false,
    "Should NOT auto-discover project-d (depends on project-a)",
  );
  assertEquals(
    taskIds.includes("./project-e:test"),
    false,
    "Should NOT auto-discover project-e (depends on project-a)",
  );
  assertEquals(
    taskIds.includes("./project-f:test"),
    false,
    "Should NOT auto-discover project-f (dependency of project-e)",
  );
});
...
Deno.test("CORRECTED: Empty dependencies behavior", () => {
  ...
  const taskIds = plan.tasks.map((t) => t.id);
  assertEquals(
    taskIds.includes("./other-project:test"),
    false,
    "Should NOT execute dependent projects",
  );
});
...
Deno.test("CORRECTED: resolveTestPattern should behave like resolve() - no client auto-discovery", () => {
  ...
  assertEquals(
    testPatternPlan.tasks.length,
    regularPlan.tasks.length,
  ...
  assertEquals(
    testPatternIds.includes("./project-d:test"),
    false,
    "resolveTestPattern should NOT auto-discover project-d",
  );
  assertEquals(
    testPatternIds.includes("./project-e:test"),
    false,
    "resolveTestPattern should NOT auto-discover project-e",
  );
  assertEquals(
    testPatternIds.includes("./project-f:test"),
    false,
